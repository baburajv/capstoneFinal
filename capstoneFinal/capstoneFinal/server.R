...
# Load the RDS files for processing
...
# PredictNext function takes a string and predict the next word based on the occurance in quadgram, trigram and bigram. 
...
PredictNext <- function(x) {
  ...
  if (length(xs)>= 3) {
    ...
    if (identical(character(0),head(quadgram[quadgram$unigram == xs[1] & quadgram$bigram == xs[2] & quadgram$trigram == xs[3], 4],1))){
      PredictNext(paste(xs[2],xs[3],sep=" "))
    }
